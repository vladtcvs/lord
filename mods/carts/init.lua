...
dofile(minetest.get_modpath("carts").."/crafting.lua")
...
function cart:on_step(dtime)
	...
	-- Limit the velocity
	if math.abs(self.velocity.x) > CART_MAX_VELOCITY then
		self.velocity.x = CART_MAX_VELOCITY*cart_func:get_sign(self.velocity.x)
	end
	if math.abs(self.velocity.y) > CART_MAX_VELOCITY then
		self.velocity.y = CART_MAX_VELOCITY*cart_func:get_sign(self.velocity.y)
	end
	if math.abs(self.velocity.z) > CART_MAX_VELOCITY then
		self.velocity.z = CART_MAX_VELOCITY*cart_func:get_sign(self.velocity.z)
	end
...
minetest.register_node("carts:accelerating_rail", {
	...
	on_construct = function(pos)
		...
		if cart_func:is_accelerator(accel_pos) then
			minetest.get_meta(pos):set_string("cart_acceleration", CART_ACCELERATION)
		else
...
minetest.register_node("carts:stopping_rail", {
